...
import { CacheSearchCriteria, ChangePage, ChangeSearchMaxResults, ChangeSearchType, InitializeSearchCriteriaFromCache, LoadMoviesList, LoadMoviesListFail, LoadMoviesListSuccess } from "../actions/movies-list.action";
...
describe("MovieListReducer", () => {
  ...
  it("CacheSearchCriteria action reducer should update localstorage", () => {
    const action = new CacheSearchCriteria();
    const page = 1;
    const searchCriteriaStringified = JSON.stringify({
    ...
    expect(localStorage.setItem).toHaveBeenCalledWith(
      "SEARCH_CRITERIA",
      searchCriteriaStringified
    );
  });

  it("InitializeSearchCriteriaFromCache action reducer should update searchCriteria", () => {
    const action = new InitializeSearchCriteriaFromCache(
      new SearchCriteria(98765, 87654, "metascore", 1)
    );
  ...
  it("ChangeSearchMaxResults action should update max results per page value and page number", () => {
    ...
    expect(state.searchCriteria.page).toEqual(1);
    expect(state.searchCriteria.sortBy).toEqual(
      initialState.searchCriteria.sortBy
    );
    expect(state.searchCriteria.sortDir).toEqual(
      initialState.searchCriteria.sortDir
    );
  });
  ...
  it("LoadMoviesListSuccess action reducder should update pageLimit", () => {
    ...
    const newLimit = Math.floor(
      action.payload.total / state.searchCriteria.limit + 1
    );
